...
import { getOrder, validateDiscountCode, selectPaymentMethod } from '../../lib/api/orders';
...
export function PaymentMethodStep() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.assign(`/checkout/confirm?orderId=${encodeURIComponent(orderId)}`);
    } catch (err) {
      const t = translateApiError(err);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <FormActions
                onBack={() => window.location.assign(`/checkout/operational-standards?orderId=${encodeURIComponent(orderId ?? '')}`)}
                submitLabel="Dalej"
